...
// "Eboard Meeting" + "Poll") — unlike CalendarScreen's grid, this list
// does include polls, exactly like it always did.
...
// The original flat Upcoming/Past scrolling list, split out onto its own
// screen (founder request: Calendar should be grid-only, and viewing the
// list of events should be its own separate destination — reached from
// chat's header menu — rather than a toggle on the Calendar screen
// itself). Merges the exact same feed CalendarScreen's grid does — see
// lib/calendarFeed.ts — including polls, which the grid deliberately
// excludes but this list always has.
export default function EventsListScreen(props: CalendarScreenProps) {
  ...
  if (loadError) {
    return <LoadError message="Couldn't load events." onRetry={() => setRetryToken((t) => t + 1)} />;
  }
