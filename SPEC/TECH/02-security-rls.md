...
| `decide_join_request` | `request_id uuid, approve boolean` | definer | Re-checks `is_club_admin`; updates status **and** inserts the membership row in one transaction; sets `clubchat.skip_add_notify` so the membership trigger doesn't fire a duplicate "added by" notification; resolves the admin-inbox notification by `target_path` + `actor_id`. **Idempotent (0082):** the status UPDATE carries `status = 'pending'` and the function returns early if it matches no row, so a second decider is a no-op (no duplicate notification; `decided_by` keeps the first) |
...
| `decide_race_join_request` | `request_id uuid, approve boolean` | definer | As `decide_join_request` (including the 0082 idempotency guard), scoped to `race_members` / `is_race_admin` |
...
| `decide_eboard_join_request` | `request_id uuid, approve boolean` | definer | Authorized by `is_eboard_member`, **not** `is_club_admin` - Eboard approval rights belong to existing members. Same 0082 idempotency guard |
