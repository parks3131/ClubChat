...
A fix that was not preceded by a reproduction is a guess. This repo has shipped
several confident guesses that were wrong, and misdiagnosed the same "infinite
spinner" twice as a stuck Supabase call when it was navigation logic. So:

1. **Reproduce end-to-end first**, the way a user actually hits it: through the
   running app, on the platform where it was reported, with realistic data. Not by
   reading code and reasoning about what is probably wrong.
2. **Only then locate the cause.** If reproduction and hypothesis disagree, the
   hypothesis is wrong. Add temporary logging and read the real sequence rather than
   re-reading plausible-looking code - that is how the `FlatList` handler-ordering
   bugs were finally found.
3. **Fix, then re-run the identical reproduction** to prove it. Keep the reproduction
   steps in the commit or in `docs/HISTORY.md`.
4. **Ask whether the same class exists elsewhere.** `role = 'admin'` shipped four
   times; the `.insert().select()` RLS trap twice. A single instance is usually a
   pattern.

Platform matters here. Web, iOS and Android have each produced bugs the other two
could not reproduce: `Alert.alert` is a no-op only on web, `fetch(uri).blob()` failed
only on Android, and `crypto.randomUUID` is missing only on Hermes.

## Quality bar

Applies to every task, not only the one that touches the area.

- **Be picky about the UI.** During any end-to-end pass, treat visible imperfection as
  a defect worth fixing on the spot: misaligned rows, inconsistent spacing, a header
  that jumps between screens, a colour that is not a `constants/theme.ts` token, a
  control a few pixels from where it belongs. Pixel perfection is the standard, and
  "unrelated to my change" is not a reason to leave it.
- **Never leave the suite red or flaky.** A failing test, a flaky test, or a type
  error gets fixed when it is seen, regardless of who caused it. Re-running until
  green is not a resolution; a flake is a real defect in either the test or the code.
- **Do not weight development cost heavily.** Prefer quality, simplicity, robustness,
  scalability and long-term maintainability over speed of delivery, and record the
  reasoning in an [ADR](../templates/adr-template.md).
...
1. **`npx tsc --noEmit` and `npm test` must both pass before any change is considered done.** CI runs exactly these two, and a red or flaky result is never acceptable as a steady state.
...
9. **Every bug fix starts with an end-to-end reproduction and ends by re-running it.** See the Bug-fix protocol above.
10. **Visible UI imperfection found during a smoke test gets fixed**, whether or not it relates to the current change.
