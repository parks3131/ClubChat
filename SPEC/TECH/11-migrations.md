...
The complete `0001`–`0082` changelog, plus the rules for adding the next file.
...
| 0082 | `guard_decide_request_reprocess` | Recreates all three `decide_*_join_request` RPCs (club, race, Eboard) so the "mark decided" UPDATE carries `status = 'pending'` and the function returns early when it matches no row | Idempotency fix (not an R-item; found while the learning session studied ACID). Two admins deciding one request re-ran the side effects: **two** `request_approved` notifications and `decided_by` = the *last* approver. The predicate in the UPDATE (not just the earlier read) makes it correct under a true concurrent race, not only the serialized case. All three siblings carried the identical bug |
